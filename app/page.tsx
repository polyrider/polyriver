  ...
  return (
    <div className="nav-panel">
      ...
      {FILTERS.map(f => (
        <button
          key={f.id}
          className={`nav-filter${activeFilter === f.id ? ' active' : ''}`}
    ...
    <div className="terminal-topbar">
      <div className="topbar-left">
        ...
        <div className="topbar-stat">
          ...
          <span className="topbar-stat-value">{eventCount}</span>
        </div>
        <div className="topbar-stat">
          <span className="topbar-stat-label">Markets</span>
          <span className="topbar-stat-value">{marketCount}</span>
        </div>
      </div>
        ...
        return (
          <div key={m.id} className="momentum-item" onClick={() => onSelect(m)}>
            ...
            <div className="momentum-bar-track">
              <div
                className={`momentum-bar-fill${isHigh ? ' high' : ''}`}
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>
    ...
    <div className="wallet-list">
      {walletEvents.slice(0, 8).map(e => (
        <a
          ...
          href={`https://polygonscan.com/address/${e.walletAddress}`}
          target="_blank"
        ...
        >
          ...
          <span className={`wallet-amount ${e.side.toLowerCase()}`}>
            {e.side} ${e.magnitude >= 1000 ? (e.magnitude / 1000).toFixed(1) + 'k' : e.magnitude.toFixed(0)}
          </span>
        </a>
      ))}
    </div>
  ...
  if (!insight) {
    return (
      <div className="insight-panel">
        <div className="state-loading" style={{ height: 40 }}>
          <div className="spinner" />
          Generating insight...
        </div>
      </div>
  ...
  return (
    <div className="insight-panel">
      ...
      <div className="insight-footer">
        ...
        <span>{relativeTime(insight.generatedAt)}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="terminal-shell">
      ...
      <div className="terminal-body">
        ...
        {/* Right – momentum + AI + wallets */}
        <div className="panel-right">
          {/* Momentum */}
          <div className="right-momentum">
            <div className="panel-header">
              <span className="panel-header-label">Market Momentum</span>
            </div>
            <div className="panel-section">
              <MomentumPanel markets={markets} onSelect={setSelectedMarket} />
            </div>
          </div>
          ...
          <div className="right-wallets">
            <div className="panel-header">
              <span className="panel-header-label">Smart Wallets</span>
            </div>
            <div className="panel-section">
              <WalletTracker events={events} />
            </div>
          </div>
        </div>
      </div>
